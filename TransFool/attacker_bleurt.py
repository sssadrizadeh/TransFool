...
class Attacker():
    ...
    def gen_adv(self, sentence_number, LM_model, fc):
        ...
        if self.args.part=="":
            ...
            part = self.args.part
        metric_bleu = load_metric("bleurt", 'bleurt-20', module_type="metric") #load_metric("sacrebleu")
        ...
        for w_sim_coef in list_w_sim:
            ...
            if 'white' in self.attack_type:
                ...
                first_tr_ids = self.black_model.generate(torch.LongTensor(black_input_ids).unsqueeze(0).to(self.device))
                first_tr = [decode(first_tr_ids[0],self.black_tokenizer,self.args.black_model_name,self.args.black_target_lang)]

            org_bleu = metric_bleu.compute(predictions=first_tr, references=ref_tr_decode[0])['scores'][0]
                ...
                if torch.equal(w_prime,w)==False:
                    ...
                    if skip==False:
                        ...
                        if 'white' in self.attack_type:
                            ...
                            output_ids = self.black_model.generate(ref_index_prime.unsqueeze(0).to(self.device))
                        ...
                        adv_bleu = metric_bleu.compute(predictions=pred_project_decode, references=ref_tr_decode[0])['scores'][0]
    ...
    def google_attack(self, sentence_number, LM_model, fc, flag):
